...
if(isset($_POST['btn']))
	{
		...
		$today = date("Y-m-d");
		$date = strtotime("+6 days", strtotime($today));
		$lastday =  date('Y-m-d', $date);

		if( ($today<= $_SESSION['journey_date']) && ($lastday >= $_SESSION['journey_date'])){
			if(date("Y-m-d") == $_SESSION['journey_date']){
				$current_time = date("H:i:s");
				$time = strtotime("+15 minutes", strtotime($current_time));
	    	...
	    	$Available_bus=$trip_info->GetSearchBus($_SESSION['from_city'],$_SESSION['to_city'],$time);
	    }
...
<div class="bus-tp">
	<div class="container">
		<p>Date of the Trip : <?php echo date('F j, Y', strtotime(Session::get("journey_date")));  ?></p>
...
<div class="bus-midd" >
	...
	<?php
	if(isset($Available_bus) && $Available_bus)
	{
